...
parser.add_argument('--size', action="store",help='image width size ')
parser.add_argument('--objprob', action="store",default='0.02',help='image width size ')
...
def detect_obj(imname):
    try:
        ...
        for i in xrange(top_conf.shape[0]):
            ...
            results.append({'index':i,'xmin':xmin,'ymin':ymin,'xmax':xmax,'ymax':ymax,'label':label_name,'score':score})
...
def detectUrl():
    ...
    if content.mode is 0:
        if content.imgUrl != '' and content.imgUrl != None:
            ret , results = detect_img_by_name(content.imgUrl)
            return jsonify({'success':ret,'img':content.imgUrl,'results':content.results})
        else:
